...
<div class="container col-md-12 m-4 justify-content-start d-flex">
    <form action="{{route('title.update', $title->id)}}" class="d-flex row ms-0" method="post">
        @csrf
        {{method_field('PUT')}}
        <label for="editName" class="ms-0">Title Name</label>
        <input required type="text" name="title_name" id="editName" class="rounded border form-control p-2 ms-2" value="{{$title->title_name}}">
